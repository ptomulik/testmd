...
* puppet is unable to find information about not yet installed ports
  ...
  - try to run manually
    ...
    if it prints something like "Please run make index", then follow this
    advice. You may also check yours ``/usr/ports`` for ``INDEX-9`` (or so)
    and if it's absent, then regenerate it with ``make index``.
